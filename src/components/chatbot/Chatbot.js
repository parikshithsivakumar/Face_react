...
const Chatbot = () => {
  ...
  useEffect(() => {
    if (Object.keys(qaData).length > 0) {
      ...
      setFuse(
        new Fuse(
          Object.entries(qaData).map(([q, a]) => ({ question: q, answer: a })),
          options
        )
      );
    }
  ...
  const addUserMessage = (msg) => {
    setMessages((prevMessages) => [
      ...prevMessages,
      { text: msg, isBot: false, id: Date.now() },
    ]);
  };
  ...
  const processUserMessage = (msg) => {
    ...
    switch (intent) {
      ...
      case "blogs":
        addBotMessage(
          "Please go to our Blog page to read more about our Technical Knowledgeable Blogs!"
        );
        break;
      case "about":
        addBotMessage(
          "Please Go to our About page to read more about FACE and our Achievements!"
        );
        break;
      default:
        if (!fuse) {
          addBotMessage(
            "I'm sorry, I won't be able to answer that. Please contact us for more information!"
          );
          return;
        }
